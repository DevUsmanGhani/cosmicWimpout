...
def game2():    
    ...
    #[This function plays a round of the dice game "Cosmic Wimpout"]
    #Test data:
    #input: ’’
    #output: ’’
    #Params: none 
    ...
    initialPoints = [0]
    play(initialDice, initialPoints)
...
def isFlash4(thing, tracker):
    ...
    def newRoll(b):
        ...
        if a == 5:
            ...
            print("Your total score is: %d" % totalPoints)
            rollAgain = "no"
            goAgain = input("Would you like to roll again?")
            if goAgain == "yes" or goAgain == "y":
                play(thing, tracker)
            else:
                thing.append(0)
...
def play(dice, points):
    while (len(dice) <= 5):
        ...
        results(dice)
        isOver(dice)
        isFreight(dice, points)
        isFlash(dice, points)
        isFlash4(dice, points)
        plainPoints(dice, points)
